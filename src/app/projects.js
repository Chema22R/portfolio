...
function addProjects(projects) {
    ...
    for (let i=0, codeHTML; i<projects.length; i++) {
        codeHTML =  `<div id='project-${projects[i].id}' class='project${device}'>` +
                    (projects[i].id === 'curriculum' ? '<div id="helpBubble"><img src="images/me.png"><p>Welcome!</p></div>' : '') +
                    (projects[i].id === 'curriculum' ? '<div>' : `<a href='${projects[i].url}'>`) +
                    '<figure>' +
        ...
        if (projects[i].id === 'curriculum') {
            $().appendTo(`#project-${projects[i].id} figcaption`);
        }
        ...
        if (projects[i].health) {
            fetch(projects[i].health).then((res) => {
                ...
                document.getElementsByClassName(`${projects[i].id} icon-${res.ok ? 'success' : 'error'}`)[0].style.display = 'unset';
            }).catch((reason) => {
                setTimeout(() => {
                    document.getElementsByClassName(`${projects[i].id} icon-loading`)[0].style.display = 'none';
...
function loadHandlers() {
    $('#project-curriculum > a').on('click', (e) => {
        e.preventDefault();
    });
    ...
    if (!window.isMobile.any()) {
        ...
        $('.project figure .projectBack .description').on('scroll', (e) => {
